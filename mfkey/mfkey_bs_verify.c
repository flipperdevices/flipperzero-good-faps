...
#define VFP_PARK(var, slot)   float slot; \
    __asm__ volatile("vmov %0, %1" : "=t"(slot) : "r"(var))
#define VFP_UNPARK(var, slot) \
    __asm__ volatile("vmov %0, %1" : "=r"(var) : "t"(slot))
#else
...
uint32_t bs_verify_batch_32_nested(
    Crypto1BitSlice* bs,
    MfClassicNonce* nonce,
    uint32_t alive)
{
    ...
    uint32_t alive)
{
    ...
    while (remaining) {
        ...
        if ((crypt_word_par(&temp, nonce->uid_xor_nt0, 0, nonce->nt0, &pk) ==
             nonce->ks1_1_enc) &&
            (pk == nonce->par_1)) {
