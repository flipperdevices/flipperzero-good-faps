...
NfcCommand
    ...
    if(event.type == Gen4PollerEventTypeCardDetected) {
        ...
        event.data->request_mode.mode = Gen4PollerModeSetATS;
...
static void nfc_magic_scene_gen4_set_ats_setup_view(NfcMagicApp* instance) {
    Popup* popup = instance->popup;
    popup_reset(popup);
    uint32_t state =
        scene_manager_get_scene_state(instance->scene_manager, NfcMagicSceneGen4SetATS);
    if(state == NfcMagicSceneGen4SetATSStateCardSearch) {
        popup_set_icon(instance->popup, 0, 8, &I_NFC_manual_60x50);
        popup_set_text(
            instance->popup, "Apply the\ncard\nto the back", 128, 32, AlignRight, AlignCenter);
    } else {
        popup_set_icon(popup, 12, 23, &I_Loading_24);
        popup_set_header(popup, "Setting ATS\nDon't move...", 52, 32, AlignLeft, AlignCenter);
    }
...
bool nfc_magic_scene_gen4_set_ats_on_event(void* context, SceneManagerEvent event) {
    NfcMagicApp* instance = context;
    bool consumed = false;
...
void nfc_magic_scene_gen4_set_ats_on_exit(void* context) {
    NfcMagicApp* instance = context;
