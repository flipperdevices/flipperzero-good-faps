...
enum {
    NfcMagicSceneGen4SetMaxRWBlockStateCardSearch,
...
NfcCommand nfc_magic_scene_gen4_set_max_rw_block_poller_callback(
    ...
    if(event.type == Gen4PollerEventTypeCardDetected) {
        ...
        event.data->request_mode.mode = Gen4PollerModeSetMaxRWBlock;
...
static void nfc_magic_scene_gen4_set_max_rw_block_setup_view(NfcMagicApp* instance) {
    Popup* popup = instance->popup;
    popup_reset(popup);
    uint32_t state =
        scene_manager_get_scene_state(instance->scene_manager, NfcMagicSceneGen4SetMaxRWBlock);

    if(state == NfcMagicSceneGen4SetMaxRWBlockStateCardSearch) {
...
void nfc_magic_scene_gen4_set_max_rw_block_on_enter(void* context) {
    NfcMagicApp* instance = context;

    scene_manager_set_scene_state(
        instance->scene_manager,
        NfcMagicSceneGen4SetMaxRWBlock,
        NfcMagicSceneGen4SetMaxRWBlockStateCardSearch);
    nfc_magic_scene_gen4_set_max_rw_block_setup_view(instance);
...
void nfc_magic_scene_gen4_set_max_rw_block_on_exit(void* context) {
    NfcMagicApp* instance = context;
