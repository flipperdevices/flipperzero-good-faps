...
enum {
    NfcMagicSceneGen4SetTypeStateCardSearch,
    NfcMagicSceneGen4SetTypeStateCardFound,
};

NfcCommand nfc_magic_scene_gen4_set_type_poller_callback(Gen4PollerEvent event, void* context) {
    ...
    if(event.type == Gen4PollerEventTypeCardDetected) {
        ...
        event.data->request_mode.mode = Gen4PollerModeSetType;
...
static void nfc_magic_scene_gen4_set_type_setup_view(NfcMagicApp* instance) {
    Popup* popup = instance->popup;
    popup_reset(popup);
    uint32_t state =
        scene_manager_get_scene_state(instance->scene_manager, NfcMagicSceneGen4SetType);

    if(state == NfcMagicSceneGen4SetTypeStateCardSearch) {
        popup_set_icon(instance->popup, 0, 8, &I_NFC_manual_60x50);
        popup_set_text(
            instance->popup, "Apply the\ncard\nto the back", 128, 32, AlignRight, AlignCenter);
    } else {
        popup_set_icon(popup, 12, 23, &I_Loading_24);
        popup_set_header(popup, "Setting Type\nDon't move...", 52, 32, AlignLeft, AlignCenter);
    }

    view_dispatcher_switch_to_view(instance->view_dispatcher, NfcMagicAppViewPopup);
}

void nfc_magic_scene_gen4_set_type_on_enter(void* context) {
    NfcMagicApp* instance = context;

    scene_manager_set_scene_state(
        instance->scene_manager,
        NfcMagicSceneGen4SetType,
        NfcMagicSceneGen4SetTypeStateCardSearch);
    nfc_magic_scene_gen4_set_type_setup_view(instance);
    ...
    gen4_poller_struct_set_tag_type(instance->gen4_poller, instance->byte_input_store[0]);

    gen4_poller_start(
        instance->gen4_poller, nfc_magic_scene_gen4_set_type_poller_callback, instance);
}

bool nfc_magic_scene_gen4_set_type_on_event(void* context, SceneManagerEvent event) {
    NfcMagicApp* instance = context;
    bool consumed = false;
...
void nfc_magic_scene_gen4_set_type_on_exit(void* context) {
    NfcMagicApp* instance = context;
